...
async function useSQLiteAuthState(databasePath) {
    // PRIEST MD bridge: SQLite auth state is not used — PRIEST MD manages
    // sessions with multi-file auth + pairing codes (see session-manager.js).
    throw new Error(
        'useSQLiteAuthState is not available in the PRIEST MD bridge — ' +
        'use the built-in session manager (sessions/<number>/) instead.'
    );
}
