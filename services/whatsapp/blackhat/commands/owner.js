...
gmd(
  ...
  async (from, Gifted, conText) => {
    ...
    try {
      ...
      // Modern Baileys API — works for groups too (auto-resizes).
      await Gifted.updateProfilePicture(from, imageBuffer);
      await react("✅");
...
gmd(
  ...
  async (from, Gifted, conText) => {
    ...
    try {
      ...
      // Modern Baileys API (auto-resizes to 640x640).
      await Gifted.updateProfilePicture(Gifted.user.id, imageBuffer);
      await react("✅");
      await fs.unlink(tempFilePath);
...
gmd(
  ...
  async (from, Gifted, conText) => {
    ...
    try {
      ...
      // Download the quoted image to a temp file (bridged helper).
      tempFilePath = await Gifted.downloadAndSaveMediaMessage(
        quotedImg,
        "temp_media",
      );
      ...
      // Modern Baileys API: updateProfilePicture(jid, imageBuffer) —
      // it auto-resizes to 640x640.
      await Gifted.updateProfilePicture(Gifted.user.id, imageBuffer);
